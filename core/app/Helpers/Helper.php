...
class Helper
{
    ...
    static function canonicalURL($page_type = "", $page_id = 0)
    {
        ...
        return config('smartend.smartend_path').$canonical_url;
    }
    ...
    static function checkLdapService($username,$password){
        $client = new Client();
        $p_username = urlencode($username);
        $p_password = urlencode($password);
        $params['headers'] = ['Content-Type' => 'application/json'];
        $params['json'] = array('username' => $p_username, 'password' => $p_password);
        $apiURL = "https://arserv2.medicare.com.vn/api/get-status-user-domain";
        try{
            $res = $client->post($apiURL, $params);
            if(strcasecmp($res->getBody()->getContents(),"True")==0) return true;
        }catch(Exception $e){
            return false;
        }
